...
class ScrapeNotificationDeliverImplTest : AbstractTest() {
    ...
    private lateinit var notificationDeliver: ScrapeNotificationDeliverImpl

    @BeforeEach
    fun setUp() {
        notificationDeliver = ScrapeNotificationDeliverImpl(sqsAsyncNotificationBroker)
    ...
    fun shouldAskSqsAsyncNotificationBrokerNotifyStringMessageContainingJsonObjectContentWithIdAndOtherPropertyValues() {
        ...
        notificationDeliver.notifyIntentedScrape(id)
    ...
    fun shouldAskSqsAsyncNotificationBrokerNotifyStringMessageContainingJsonObjectContentBasedOnRetryScrapeNotificationAndItsPropertiesAndValues() {
        val id = Id.of(defaultId)
        val now = Clock.System.now().toLocalDateTime(TimeZone.currentSystemDefault())
        val notification = ScrapeNotificationDTO(id, retryCount = 1, firstAttempt = now)
